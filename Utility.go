...
var idMaker *redis.IdMaker

func MakeId(size int) string {
	if idMaker == nil {
		return u.DefaultIdMaker.Get(size)
	}
	return idMaker.Get(size)
}

func MakeIdForMysql(size int) string {
	if idMaker == nil {
		return u.DefaultIdMaker.GetForMysql(size)
	}
	return idMaker.GetForMysql(size)
}

func MakeIdForPostgreSQL(size int) string {
	if idMaker == nil {
		return u.DefaultIdMaker.GetForPostgreSQL(size)
	}
	return idMaker.GetForPostgreSQL(size)
}
