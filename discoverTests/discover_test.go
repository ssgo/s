...
func TestBase(tt *testing.T) {
	...
	s.Proxy(1, "/dc1/s1", "a1", "/dc/s1")
	s.Proxy(1, "/dc2/(.+?)", "a1", "/dc/$1")

	s.Rewrite("/rrr/123", "/dc/c1")
	s.Rewrite("/r2/(.+?)/aa", "/dc/$1")
	s.Rewrite("/r3\\?name=(\\w+)", "/dc/$1")
	s.Rewrite("/bd", "http://www.baidu.com/")
	...
	//bd := as.Get("/bd", "Access-Token", "aabbcc").String()
	//t.Test(r.Error == nil && strings.Contains(bd, "baidu"), "DC by rewrite baidu", r.Error, bd)
}
